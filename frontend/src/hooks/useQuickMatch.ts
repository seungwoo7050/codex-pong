 ...
 */
export function useQuickMatch(token?: string | null) {
  ...
  const start = async () => {
    ...
    try {
      ...
      const response = await apiFetch<MatchmakingResponse>(
        '/api/match/quick',
        { method: 'POST' },
        token,
      )
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      try {
        const result = await apiFetch<MatchmakingResponse>(
          `/api/match/quick/${ticketId}`,
          { method: 'GET' },
          token,
        )
