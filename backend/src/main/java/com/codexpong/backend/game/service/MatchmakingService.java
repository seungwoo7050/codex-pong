...
public class MatchmakingService {
     ...
     */
    public MatchTicket enqueue(User user) {
        Optional<MatchTicket> existing = tickets.values().stream()
                .filter(ticket -> ticket.userId().equals(user.getId()) && !ticket.status().equals("CANCELLED"))
                .findFirst();
        ...
        if (opponent != null && !opponent.getId().equals(user.getId())) {
            GameRoom room = gameRoomService.createRoom(opponent, user);
            MatchTicket opponentTicket = new MatchTicket(UUID.randomUUID().toString(), opponent.getId(), "MATCHED",
                    room.getRoomId());
            tickets.put(opponentTicket.ticketId(), opponentTicket);
            MatchTicket myTicket = new MatchTicket(UUID.randomUUID().toString(), user.getId(), "MATCHED", room.getRoomId());
            tickets.put(myTicket.ticketId(), myTicket);
            return myTicket;
        }
        waitingQueue.offer(user);
        MatchTicket ticket = new MatchTicket(UUID.randomUUID().toString(), user.getId(), "WAITING", null);
    ...
    public record MatchTicket(String ticketId, Long userId, String status, String roomId) {
    }
}
